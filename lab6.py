...
#this method checks if s only has one set
def one_set(S):
    count = 0
    for i in S:#everytime we find a -1, that means we have a single set, if we have more than one, it is false.
        if i == -1:
            count += 1
    if(count > 1):
        return False
...
#iterates through the Forest and stops once we have only 1 set in the DSF
